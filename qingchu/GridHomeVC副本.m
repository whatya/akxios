...
- (void)loginToServer
{
    ...
    NSArray *keys = @[@"user",@"pwd",@"regid",@"apikey",@"secretkey",@"devicetype",@"oemLoginBgImage",@"oemHomeBgImage",@"oemTitle"];
    NSArray *values = @[username ?: @"",
                        [MyMD5 md5:password] ?: @"",
                        [[NSPublic shareInstance] getUserTXId] ?: @"" ,
                        @"9085bca5773959465f5c933c",
                        @"2330451c7921a0e5c9ed1f92",
                        @"4",@"http://img.3chunhui.com/xx.png",@"http://img.3chunhui.com/xx.png",@"安康信"];
...
//                UIImageView *homgBgImg = (UIImageView*)[self.view viewWithTag:100];
                ...
                NSString *imgString = jsonData[@"data"][@"oem"][@"oemHomeBgImage"];
...
- (void)navigateTo:(int)index withParameters:(NSDictionary*)dictionary animation:(BOOL)animate
{
    ...
    switch (index) {
        ...
        case 5:
            ...
            if (self.focusPersonArray.count > 0) {
                PUSH(@"Home", @"NoteListVC", @"健康报警", @{},animate);
            }else{
                PUSH(@"More", @"AddFocusTVCForWX", @"添加关注人", @{},YES);
            }
        ...
        case 6:
            ...
            PUSH(@"Mall", @"ItemsVC", @"", @{},animate);
//            UIStoryboard *storyboard = [UIStoryboard storyboardWithName:@"Mall" bundle:nil];
//            UIViewController *vc = [UIStoryboard instantiateViewControllerWithIdentifier:@"ItemsVC"];
//            
